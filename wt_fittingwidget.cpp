 ...
 * 文件作用: 试井拟合分析主界面实现文件
 * 修改记录:
 * 1. [界面优化] 确保三个子窗口 (LogLog, SemiLog, Cartesian) 设置了明确的窗口标题，以便最小化时显示图表类型名称。
 * 2. [代码兼容] 适配了 ChartWidget 的按钮移除修改，保留了原有的功能调用。
 */
    ...
    m_isCustomSamplingEnabled(false)
{
    ...
    m_chartLogLog = new ChartWidget(this);
    m_chartSemiLog = new ChartWidget(this);
    m_chartCartesian = new ChartWidget(this);
    ...
    // [修改] 设置子窗口标题 (最小化时显示此名称)
    m_subWinLogLog->setWindowTitle("双对数图");
    m_subWinSemiLog->setWindowTitle("半对数图");
    m_subWinCartesian->setWindowTitle("标准坐标系");

    connect(m_chartLogLog, &ChartWidget::exportDataTriggered, this, &FittingWidget::onExportCurveData);
...
void FittingWidget::updateModelCurve(const QMap<QString, double>* explicitParams, bool autoScale, bool calcError) {
    ...
    if(m_obsTime.isEmpty()) {
        m_plotLogLog->clearGraphs(); m_plotLogLog->replot();
        m_plotSemiLog->clearGraphs(); m_plotSemiLog->replot();
        m_plotCartesian->clearGraphs(); m_plotCartesian->replot();
        return;
    }
